...
async function run() {
  ...
  app.post("/admin/create-staff", verifyJWT, verifyAdmin, async (req, res) => {
  ...
  //  REJECT ISSUE (ADMIN)
  app.patch("/issues/:id/reject", verifyJWT, verifyAdmin, async (req, res) => {
    try {
      const id = req.params.id;
      const issue = await issuesCollection.findOne({ _id: new ObjectId(id) });
      if (!issue) return res.status(404).send({ error: "Issue not found" });
  ...
  app.get("/citizen/stats", verifyJWT, async (req, res) => {
    try {
      ...
      const totalPayments = await paymentsCollection.countDocuments({ email });
